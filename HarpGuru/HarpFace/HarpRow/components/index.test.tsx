...
test.skip('A blow row from a major diatonic harmonica can be rendered', () => {
...
test.skip('A snapshot of a non-home row', () => {
  const { container } = render(<HarpRow {...harpFaceProps} yCoord={0} />)

  expect(container).toMatchSnapshot()
})

test.skip('A snapshot of a blow home row', () => {
  const { container } = render(<HarpRow {...harpFaceProps} yCoord={2} />)

  expect(container).toMatchSnapshot()
})

test.skip('A snapshot of a draw home row', () => {
