...
test('leftmost is true if the provided coord has is 0 in the x axis and false otherwise', () => {
  ...
  const { leftmost: leftmostTrue } = analysePosition({ ...harpFaceProps, yxCoord: yxCoordLeftmostTrue})
  const { leftmost: leftmostFalse } = analysePosition({ ...harpFaceProps, yxCoord: yxCoordLeftmostFalse})
...
test('thisDegree, thisPitch, thisInteraction, & thisIsActive provide the degree, pitch, interaction and isActive at this position', () => {
  ...
  const { thisDegree, thisPitch, thisInteraction, thisIsActive } = analysePosition({ ...harpFaceProps, yxCoord: ourCoord})
