...
test('Use of same hook from different components results in totally isolated ref values', () => {
  const { getByText: getByText_1, rerender: rerender_1 } = render(
    <TestingComponent value={'value for #1'} initial={'initial for #1'} />
  )
  const { getByText: getByText_2, rerender: rerender_2 } = render(
    <TestingComponent value={'value for #2'} initial={'initial for #2'} />
  )
  const { getByText: getByText_3 } = render(
